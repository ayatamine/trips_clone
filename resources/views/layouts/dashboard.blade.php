...
<body class="hold-transition dark-mode sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
  <div class="wrapper">
    ...
    <div class="preloader flex-column justify-content-center align-items-center">
      <img class="animation__wobble" src="{{asset('theme/admin/dist/img/AdminLTELogo.png')}}" alt="AdminLTELogo"
        height="60" width="60">
    </div>
      ...
      <ul class="navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown dropdown-notifications">
          ...
          <a class="nav-link" data-toggle="dropdown">
            ...
            <span data-count="{{$notifications->count()}}"
              class="badge badge-danger navbar-badge notif-count">{{$notifications->count()}}</span>
          </a>
                ...
                <div dir="rtl" align="right" class="media">
                  <div class="media-body">
                    <h3 class="dropdown-item-title">
                      <p class="text-sm text-muted"><span dir="ltr" align="left" style="float: left;"><span
                            id="notiDate{{$not->id}}" class=" mr-1"> {{$not->updated_at->diffForHumans()}}</span></span>
                        <span id="notiName{{$not->id}}">{{$not->name}}</span></p>
                    </h3>
                    <p class="text-sm"><span id="notiCount{{$not->id}}"
                        class="left badge badge-success">{{$not->step_number}}</span> <span id="notiPage{{$not->id}}">
                        {{$not->page}}</span></p>
    ...
    padding-left: unset;" dir="rtl" align="right" class="main-sidebar sidebar-dark-primary elevation-4">
      ...
      <div class="sidebar">
        ...
        <!-- Sidebar Menu -->
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column navright" data-widget="treeview" role="menu"
            data-accordion="false">
            <!-- Add icons to the links using the .nav-icon class
               with font-awesome or any other icon font library -->
            <li class="nav-item">
              <a href="{{route('dashboard')}}"
                class="nav-link @if(Request::segment(1) == 'dashboard' && Request::segment(2) == '' ) active @endif ">
            ...
            <li class="nav-item">
              <a href="{{route('notifications.index')}}"
                class="nav-link @if(Request::segment(2) == 'notifications') active @endif ">
                <i class="nav-icon fas fa-bell"></i>
                <p>
                  الإشعارات
                  <span data-count="{{$notifications->count()}}"
                    class="left badge badge-danger notif-count2">{{$notifications->count()}}</span>
                </p>
              </a>
            </li>
            <li class="nav-item">
